...
async function createUserGroup({ name, createdBy }) {
...
// Only analysts/guests can belong to a group (same rule as the PM-assigns path
// in services/users.js), so check it up front for a clear message rather than
// letting the request sit forever un-approvable.
async function createJoinRequest(groupId, requestedBy, requesterRole, message) {
  ...
  if (!['analyst', 'guest'].includes(requesterRole)) {
    throw new ForbiddenError('Only analysts and guests can join a user group.');
  }
...
async function decideJoinRequest(requestId, { decidedBy, decision }) {
  return db.withTransaction(async (tx) => {
    ...
    if (decision === 'approved') {
      ...
      if (!profile || !['analyst', 'guest'].includes(profile.role)) {
        throw new ForbiddenError('Only analysts and guests can be assigned to a user group.');
      }
